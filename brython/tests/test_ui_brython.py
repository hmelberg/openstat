...
# FØR fasade-lastingen (samme mekanisme i alle tre fasade-suitene). Denne
# fila ligger ett nivå dypere (brython/tests/) enn tests/test_ui_module.py,
# derfor parents[2] her (ikke parents[1]).
_SHARED = str(pathlib.Path(__file__).resolve().parents[2] / "shared")
...
# seksjon byte for byte, MED ett dokumentert dialektavvik: INGEN
# create_proxy noe sted her - se ui_brython.py sine docstrings).
# ═══════════════════════════════════════════════════════════════════════
...
def test_ui_value_real_js_null_returns_none(monkeypatch):
    # Etterligner Brythons ekte NullType-sentinel (browser-verifisert
    # 2026-07-17, Task 3-browserverifisering: en RÅ JS `null` blir en
    # instans av Brythons interne `javascript.NullType` - IKKE Python None
    # - se ui_brython.py sin _is_js_null, som gjør et rent NAVNE-basert
    # oppslag). En klasse literalt navngitt "NullType" her er alt som
    # trengs for å treffe akkurat den sjekken.
    class NullType:
        def __bool__(self):
            return False
    mod, fake = _load_ui(monkeypatch)
    fake.value_store = {"n": NullType()}
...
# Task 4). Speiler tests/test_ui_module.py sin Task 4-seksjon byte for byte
# (samme scenarier) — se der for kommentarene.
# ══════════════════════════════════════════════════════════════════════════
...
def test_generic_namespace_has_no_accepts_validation(monkeypatch):
    ...
    warned = []
    monkeypatch.setattr(mod, "_warn", warned.append)
    mod.acme.select(mod.html.div("x"))
    assert warned == []
...
# (speiler tests/test_ui_module.py, med ÉN dialektforskjell: image() sin
# figur-gren bruker _figure_spec-duck-typing i stedet for pyodide sin
# _mpl_image PNG-konvertering — se ui_brython.py sin image()-docstring)
# ══════════════════════════════════════════════════════════════════════════
...
class _FakePlotlyFigure:
    """Duck-typer plotly_express_brython.PlotlyFigure akkurat nok for
    _figure_spec: KUN to_plotly_json_str() (ikke to_plotly_json()/to_dict()
    — se _figure_spec sin docstring om hvorfor rekkefølgen betyr noe)."""
...
def test_image_matplotlib_shim_figur_rendres_nativt_som_figure_ikke_png(monkeypatch):
    # Dialektavvik fra pyodide (som konverterer til en PNG data-URI): denne
    # motoren har ingen ekte matplotlib-rasterisering — en figur-aktig
    # (PlotlyFigure, typisk fra matplotlib_brython.gcf()) rendres i stedet
    # NATIVT som en plotly-figur (kind "figure"), portert fra
    # brython/dash.py sin egen _figure_spec-håndtering.
    mod, fake = _load_ui(monkeypatch)
...
# for stripa (fase 4b, spec 2026-07-21, task-2-brief.md). Speiler
# tests/test_ui_module.py sin into=-seksjon BYTE FOR BYTE (samme API).
# Task 1 (js/ui.js) sin utvidede registerControl-retur ({"__into": true,
# "value":..., "key":..., "name":...}) og Ui.widgetValue konsumeres via
