...
**Status:** revidert v2, 2026-07-24. v1 hadde tre åpne beslutningspunkter; Hans har avgjort alle
...
**Filer:** `index.html`
- [ ] `matchHybridMarker` (7062): behold gjenkjenningen av `microdata|micro`-markøren, men la
...
- [ ] `parseHybridScript` default-fallback (7074): `'microdata'` → `'pyodide'`; r-hybrid-default
...
- [ ] `#options.mode`-parsingen (~2050 + gyldig-modus-listene ~1552, 10926): fjern `microdata`
...
- [ ] Cells-kind-klassifisering: bekreft at notatbok-celler med `## microdata`-header treffer
      samme harde feil ved kjøring (via segmentløkka) — ingen egen stille sti.
- [ ] Verifiser: script med `#micro`-blokk gir feilmeldingen i output (alle moduser); script med
...
**Filer:** `index.html`
- [ ] Fjern `modeRegistry.microdata` (~3269–3272), `RUNTIME_FOR_MODE.microdata` (3567) og
      `microdata: ''` i `editorContent` (4034).
- [ ] `restoreEditorMode()` (4059): behold vakten mot lagret `md_editor_mode === 'microdata'`
      (nå død modus, faller til hostname-default), forenkle kommentaren.
- [ ] Verifiser: ingen sti kan aktivere microdata-modus; oppstart, modusbytte og eksempellasting
      fungerer.

### Task 3 — Fjern avsløringskontroll (knapp, wiring, direktiv-strip)
**Filer:** `index.html`
- [ ] Fjern `<button id="menuDisclosureControl">` (438) + DC-wiring (`DC_KEY`/`getDisclosureControl`/
...
- [ ] **Direktiv-strip (beslutning 2):** i kjørestien(e) som sender script-tekst til motoren,
...
- [ ] Fjern hint-strengene: 1120 (avsløringskontroll-tips), 1121 (`// m2py: dc=off`-tips)
      + tilhørende i18n-nøkler.
- [ ] Verifiser: ingen DC-knapp; `// m2py: dc=on` i et script har ingen effekt; øvrige
...
- [ ] Dataminimering: fjern `<button id="btnDmQuick">` (329), `dmOptionsBackdrop`-modalen
      (~12257–12300), `dmOptionsState`/modal-wiring (631–695), `runDmVurder`/`runDmQuick` og
      dm-kallene i `js/ai-chat.js` + i18n.
- [ ] Offline-oversetter: fjern `<button id="menuOfflineBtn">` (47) + «Vis offline Python»-flyten
...
- [ ] Kjøremodus-toggel: fjern `<button id="menuRunnerMode">` + `RUNNER_MODE_KEY`-blokken
      (1859–1890) + konsumentene av `getRunnerModeFromStorage()` (~8587) — startspråk er alltid
      python.
- [ ] AI-ruting: i `js/ai-chat.js` fjern microdata-grenen i `sendCurrent()`/`sendMessage`
...
- [ ] Verifiser: knappene borte, ingen konsollfeil om manglende elementer, AI svarer via
...
- [ ] **Anbefalt løsning (a): `# load`-direktiver i statx-modus.** `DataLoader.resolveAndFetchLoads`
...
- [ ] **Tillegg (b), valgfritt:** `use NAME` som ikke finnes i `e.datasets` slår opp NAME blant
...
- [ ] Fjern micro-segment-kjøringen i `runStatxScript` (microCode-blokken) — erstattet av Task 1-
      feilen + den nye dataveien.
- [ ] Verifiser: omskrevet statx-eksempel (Task 6) kjører: `# load` → `use` → `summarize`/
...
- [ ] Slett hele `microdata`-kategorien fra `examples/manifest.json` (95 oppføringer) og
      `examples/microdata/`-katalogen. (KUN openstat — søsterrepoene beholder sine.)
- [ ] Skriv om de 14 hybrid-eksemplene til åpne-data-mønsteret:
...
- [ ] `tests/js/example-loads.test.js` + `tests/test_examples_manifest.py` kjøres og oppdateres
      (de parser ekte eksempelfiler/manifest).
- [ ] Verifiser: eksempel-menyen har ingen microdata-kategori; alle omskrevne eksempler kjører
      grønt i nettleser.

### Task 7 — Fjern safestat-/restricted-rester (beslutning 3)
**Filer:** `index.html`, `js/notebook-links.js`, `js/i18n/en.js`
- [ ] Fjern `safestat: 'pyodide'` fra `RUNTIME_FOR_MODE` (3569) — død nøkkel uten registry-oppføring.
- [ ] Velkomst: fjern `welcomeBodySafe`-markupen (268–271), forenkle `_bodies`-map (1142–1143) og
...
- [ ] `js/notebook-links.js`: `welcomeVariant` returnerer alltid `'openstat_general'` (fjern
      safestat-grenen, 78); `autorunNeedsGate` beholder KUN `!!hasSecret` (fjern
      `app === 'safestat'`-grenen, 107).
- [ ] Fjern SafeStat-velkomst-i18n-nøkler; oppdater `tests/js/notebook-links.test.js`
      (asserterer i dag safestat-varianter).
- [ ] Verifiser: velkomstmodalen viser OpenStat-varianten; autorun-gaten trigges fortsatt av
      lagrede hemmeligheter.

### Task 8 — Opprydding + verifisering
**Filer:** `index.html`, `js/i18n/en.js`
- [ ] Døde localStorage-nøkler (`microdata_runner_mode`, `microdata_disclosure_control`):
      valgfri engangs-sletting ved oppstart; ellers inerte.
- [ ] Grep-sanity: `data-mode-only="microdata"` = 0 treff; `grep -c microdata index.html` faller
      kraftig (fra ~283) — rest skal være statx-katalog/`microdataHandleTab`/
      `ensureMicrodataCatalog`/historiske kommentarer.
- [ ] Foreldreløse i18n-nøkler fjernes.
- [ ] `node --test 'tests/js/*.test.js'` grønn (oppdater tester som asserterer fjernet oppførsel);
      `python3 -m pytest tests/ -q` grønn (motor urørt — forvent uendret).
- [ ] Nettleser: python/r/duckdb/statx/jamovi/brython kjører; statx-autocomplete virker;
      Norge-kart + DuckDB-demodata virker; `#micro`-script og `#options.mode = microdata` gir
      den nye feilmeldingen; ingen konsollfeil.
- [ ] Bump `M2PY_VERSION` og push.
