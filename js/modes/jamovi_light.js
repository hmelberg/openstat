(function(){ 'use strict'; var M = window.M2PY;
    ...
    function jamoviComputeVarDialog() {
      ...
      var backdrop = document.createElement('div'); backdrop.className = 'jmvl-dialog-backdrop';
      var dlg = document.createElement('div'); dlg.className = 'jmvl-dialog'; dlg.style.maxWidth = '560px';
      dlg.innerHTML = '<div class="jmvl-dialog-head">' + T('Beregn variabel') + '</div>';
      var body = document.createElement('div'); body.className = 'jmvl-dialog-body'; body.style.display = 'block';
      body.innerHTML = '<div class="jmvl-cv-label">' + T('Nytt variabelnavn') + '</div>'
        + '<input class="jmvl-cv-input" id="jmvCvName" placeholder="f.eks. logInntekt">'
        + '<div class="jmvl-cv-label">' + T('Uttrykk (pandas)') + '</div>'
        + '<input class="jmvl-cv-input" id="jmvCvExpr" placeholder="f.eks. inntekt / 1000">'
        + '<div class="jmvl-ribbon-hint" style="margin-top:8px">' + T('Tilgjengelige kolonner: {cols}. Bruk backticks for navn med punktum/mellomrom (f.eks. `dan.sleep` * 2).', { cols: cols.map(function(c){ return M.escapeHtml(c); }).join(', ') }) + '</div>'
        + '<div id="jmvCvErr" style="color:#b91c1c;margin-top:6px;font-size:12px"></div>';
      dlg.appendChild(body);
      var foot = document.createElement('div'); foot.className = 'jmvl-dialog-foot';
    ...
    function jamoviFilterDialog() {
      ...
      var backdrop = document.createElement('div'); backdrop.className = 'jmvl-dialog-backdrop';
      var dlg = document.createElement('div'); dlg.className = 'jmvl-dialog'; dlg.style.maxWidth = '560px';
      dlg.innerHTML = '<div class="jmvl-dialog-head">' + T('Filter — påvirker analysene') + '</div>';
      var body = document.createElement('div'); body.className = 'jmvl-dialog-body'; body.style.display = 'block';
      body.innerHTML = '<div class="jmvl-cv-label">' + T('Filteruttrykk (pandas query)') + '</div>'
        + '<input class="jmvl-cv-input" id="jmvFiltExpr" placeholder="f.eks. grade > 70">'
        + '<div class="jmvl-ribbon-hint" style="margin-top:8px">' + T('Analysene kjøres på radene som oppfyller uttrykket (dataene endres ikke). Tomt = ingen filter. Kolonner: {cols}. Backticks for navn med punktum.', { cols: cols.map(function(c){ return M.escapeHtml(c); }).join(', ') }) + '</div>'
    ...
    function jamoviRecodeDialog() {
      ...
      var backdrop = document.createElement('div'); backdrop.className = 'jmvl-dialog-backdrop';
      var dlg = document.createElement('div'); dlg.className = 'jmvl-dialog'; dlg.style.maxWidth = '560px';
      dlg.innerHTML = '<div class="jmvl-dialog-head">' + T('Omkod variabel (inndeling i grupper)') + '</div>';
      var body = document.createElement('div'); body.className = 'jmvl-dialog-body'; body.style.display = 'block';
      body.innerHTML = '<div class="jmvl-cv-label">' + T('Kildevariabel') + '</div>'
        + '<select class="jmvl-cv-input" id="jmvRcSrc">' + vars.map(function(v){ return '<option>' + M.escapeHtml(v.name) + '</option>'; }).join('') + '</select>'
        + '<div class="jmvl-cv-label">' + T('Nytt variabelnavn') + '</div>'
        + '<input class="jmvl-cv-input" id="jmvRcName" placeholder="f.eks. gradeGruppe">'
        + '<div class="jmvl-cv-label">' + T('Grenser (kommaseparert)') + '</div>'
        + '<input class="jmvl-cv-input" id="jmvRcCuts" placeholder="f.eks. 0, 60, 75, 100">'
        + '<div class="jmvl-cv-label">' + T('Etiketter (én færre enn grenser)') + '</div>'
        + '<input class="jmvl-cv-input" id="jmvRcLabels" placeholder="' + T('f.eks. Lav, Middels, Høy') + '">'
    ...
    async function jamoviRefreshDatasetPicker() {
      var sel = document.getElementById('jamoviLightDatasetSelect');
      if (!sel) return;
      try {
        var py = await M.loadPyodideAndM2py();
    ...
    async function jamoviSwitchDataset(name) {
      ...
      var at = (document.querySelector('#jamoviLightRibbon .jmvl-tab.active') || {}).getAttribute && document.querySelector('#jamoviLightRibbon .jmvl-tab.active').getAttribute('data-jtab');
    ...
    function jamoviAppendPlot(title, bitmap, target) {
      ...
      block.className = 'jmvl-plot-block';
      if (title) {
        var h = document.createElement('h3');
        h.className = 'jmvl-result-title';
        h.textContent = title;
        block.appendChild(h);
      }
      ...
      canvas.className = 'jmvl-plot-canvas';
    ...
    function jamoviLightResultsContainer() {
      var c = M.outputArea.querySelector('#jamoviLightResults');
      if (!c) { M.outputArea.innerHTML = ''; c = document.createElement('div'); c.id = 'jamoviLightResults'; M.outputArea.appendChild(c); }
      return c;
    }
    ...
    function jamoviTitleCard(title) {
      var card = document.createElement('div'); card.className = 'jmvl-result-card';
      var rm = document.createElement('button'); rm.className = 'jmvl-card-remove'; rm.title = T('Fjern'); rm.textContent = '✕';
      rm.addEventListener('click', function() { card.remove(); });
      card.appendChild(rm);
      var wrap = document.createElement('div'); wrap.style.cssText = 'padding:12px 18px;';
      var h = document.createElement('h3'); h.className = 'jmvl-result-title'; h.textContent = title; wrap.appendChild(h);
      card.appendChild(wrap);
      jamoviLightResultsContainer().appendChild(card);
    ...
    function jamoviSingletonCard(id, title) {
      var container = jamoviLightResultsContainer();
      var old = document.getElementById(id);
      if (old) old.remove();
      var card = document.createElement('div'); card.className = 'jmvl-result-card'; card.id = id;
      var rm = document.createElement('button'); rm.className = 'jmvl-card-remove'; rm.title = T('Lukk'); rm.textContent = '✕';
      rm.addEventListener('click', function() { card.remove(); });
      card.appendChild(rm);
      var wrap = document.createElement('div'); wrap.style.cssText = 'padding:12px 18px;';
      var h = document.createElement('h3'); h.className = 'jmvl-result-title'; h.textContent = title; wrap.appendChild(h);
    ...
    function renderVariablesView() {
      var wrap = jamoviSingletonCard('jamoviLightVarCard', 'Variabler');
      ...
      var name = window.activeDatasetName;
      var table = document.createElement('table'); table.className = 'jmvl-result-table';
      ...
      vars.forEach(function(v) {
        ...
        var btn = document.createElement('button'); btn.type = 'button'; btn.className = 'jmvl-measure-btn';
      ...
      var note = document.createElement('div'); note.className = 'jmvl-result-note';
    ...
    async function renderDataView(focusRowId) {
      var wrap = jamoviSingletonCard('jamoviLightDataCard', 'Data');
      ...
      try {
        ...
        var d = JSON.parse(json);
        loading.remove();
        var info = document.createElement('div'); info.className = 'jmvl-result-note';
        info.innerHTML = '<i>Note.</i> ' + T('{n} rader', { n: d.n.toLocaleString(window.M2PY_LANG === 'en' ? 'en' : 'no') }) + (d.shown < d.n ? T(' (viser de første {n})', { n: d.shown }) : '') + T('. Klikk en celle for å redigere — endringer lagres i økten, men nullstilles hvis du kjører et skript på nytt.')
          + (jamoviFilter ? ' <b style="color:var(--jmvl-blue-d)">· Filter aktivt: ' + M.escapeHtml(jamoviFilter) + ' (analysene bruker bare rader som oppfyller dette)</b>' : '');
        wrap.appendChild(info);
        var gridDiv = document.createElement('div'); gridDiv.className = 'jmvl-data-grid'; wrap.appendChild(gridDiv);
        var columns = [{ title: '#', field: '__rowid__', width: 56, headerSort: false, editor: false, cssClass: 'jmvl-rowid-col' }];
    ...
    function openJamoviExamplePicker() {
      var backdrop = document.createElement('div'); backdrop.className = 'jmvl-dialog-backdrop';
      var dlg = document.createElement('div'); dlg.className = 'jmvl-dialog'; dlg.style.maxWidth = '540px';
      var head = document.createElement('div'); head.className = 'jmvl-dialog-head'; head.textContent = T('Eksempeldatasett — Learning Statistics with jamovi'); dlg.appendChild(head);
      var body = document.createElement('div'); body.className = 'jmvl-dialog-body'; body.style.display = 'block';
      var ul = document.createElement('ul'); ul.className = 'jmvl-example-list';
    ...
    function renderJamoviResult(title, struct, note) {
      ...
      function buildTable(t, heading) {
        ...
        table.className = 'jmvl-result-table';
      ...
      if (note) {
        ...
        noteEl.className = 'jmvl-result-note';
      ...
      card.className = 'jmvl-result-card';
      var rm = document.createElement('button');
      rm.className = 'jmvl-card-remove';
    ...
    function jamoviTypeIcon(type) {
      if (type === 'numeric') // continuous: jamovi's gold ruler
        return '<svg class="jmvl-type-icon" viewBox="0 0 16 16" width="14" height="14" aria-hidden="true"><rect x="1.5" y="5.5" width="13" height="5" rx="0.5" fill="#f1bf63" stroke="#cd8500" stroke-width="1"/><path d="M4 5.5v2M6.5 5.5v3M9 5.5v2M11.5 5.5v3" stroke="#cd8500" stroke-width="0.9"/></svg>';
      // nominal: jamovi's three balls (two blue + one gold)
      return '<svg class="jmvl-type-icon" viewBox="0 0 16 16" width="14" height="14" aria-hidden="true"><circle cx="5.5" cy="6.7" r="3.1" fill="#a9c6f2" stroke="#226ddd" stroke-width="1"/><circle cx="10.5" cy="6.7" r="3.1" fill="#6b9de8" stroke="#226ddd" stroke-width="1"/><circle cx="8" cy="10.6" r="3.1" fill="#f1bf63" stroke="#bf7c00" stroke-width="1"/></svg>';
    }
    ...
    function openJamoviAnalysis(id) {
      ...
      backdrop.className = 'jmvl-dialog-backdrop';

      var dialog = document.createElement('div');
      dialog.className = 'jmvl-dialog';
      dialog.setAttribute('role', 'dialog');
      dialog.setAttribute('aria-modal', 'true');

      var head = document.createElement('div');
      head.className = 'jmvl-dialog-head';
      head.textContent = spec.title;
      dialog.appendChild(head);

      var body = document.createElement('div');
      body.className = 'jmvl-dialog-body';
      dialog.appendChild(body);

      var foot = document.createElement('div');
      foot.className = 'jmvl-dialog-foot';
      ...
      if (!vars.length) {
        ...
        varlistDiv.className = 'jmvl-varlist';
        var varlistLabel = document.createElement('div');
        varlistLabel.className = 'jmvl-role-label';
        varlistLabel.textContent = T('Variabler');
        varlistDiv.appendChild(varlistLabel);
        var varFilter = '';
        var search = document.createElement('input');
        search.type = 'text'; search.className = 'jmvl-var-search'; search.placeholder = T('Søk variabel…');
        ...
        spec.roles.forEach(function(roleSpec){
          ...
          box.className = 'jmvl-rolebox' + (roleSpec.key === activeRoleKey ? ' active' : '');
        ...
        function refreshVarList(){
          ...
          vars.forEach(function(v){
            ...
            var li = document.createElement('li');
            li.innerHTML = jamoviTypeIcon(v.type) + '<span class="jmvl-var-name">' + M.escapeHtml(v.name) + '</span>';
            li.dataset.varname = v.name; li.dataset.vartype = v.type;
            if (selectedVar === v.name) li.className = 'jmvl-selected';
        ...
        function refreshRoles(){
          spec.roles.forEach(function(rs){
            ...
            box.className = 'jmvl-rolebox' + (rs.key === activeRoleKey ? ' active' : '');
            box.innerHTML = '';
            (assignments[rs.key] || []).forEach(function(varname){
              var li = document.createElement('li');
              li.innerHTML = jamoviTypeIcon(typeOf[varname]) + '<span class="jmvl-var-name">' + M.escapeHtml(varname) + '</span><span class="jmvl-remove">✕</span>';
        ...
        if (spec.optionSections && spec.optionSections.length && vars.length) {
          spec.optionSections.forEach(function(sec, sIdx){
            ...
            var secEl = document.createElement('div'); secEl.className = 'jmvl-section' + (collapsed ? ' collapsed' : '');
            var hdr = document.createElement('div'); hdr.className = 'jmvl-section-hdr';
            hdr.innerHTML = '<span class="jmvl-section-caret">▾</span><span class="jmvl-section-title">' + sec.title + '</span>';
            hdr.addEventListener('click', function(){ secEl.classList.toggle('collapsed'); });
            var bodyEl = document.createElement('div'); bodyEl.className = 'jmvl-section-body';
            sec.groups.forEach(function(g){
              var gEl = document.createElement('div'); gEl.className = 'jmvl-opt-group';
              if (g.title) { var gh = document.createElement('div'); gh.className = 'jmvl-opt-grouphdr'; gh.textContent = g.title; gEl.appendChild(gh); }
              g.items.forEach(function(it){
                optsObj[it.key] = Array.isArray(it.default) ? it.default.slice() : it.default;
                if (it.type === 'check') {
                  var lab = document.createElement('label'); lab.className = 'jmvl-opt-item';
                  ...
                  var wrap = document.createElement('label'); wrap.className = 'jmvl-opt-item';
                  if (it.label) wrap.appendChild(document.createTextNode(it.label + ' '));
                  var sel = document.createElement('select'); sel.className = 'jmvl-opt-select';
    ...
    if (bar && !document.getElementById('jamoviLightRibbon')) {
      var rib = document.createElement('div');
      rib.id = 'jamoviLightRibbon'; rib.className = 'jamovi-light-ribbon'; rib.setAttribute('data-mode-gui','jamovilight'); rib.setAttribute('aria-label','Jamovi light');
      var catGroups = '<div class="jmvl-group"><button type="button" class="jmvl-cat" data-cat="exploration">Exploration</button>\n            <div class="jmvl-menu"><button type="button" data-an="descriptives">Descriptives</button><button type="button" data-an="frequencies">Frequencies</button></div></div>\n          <div class="jmvl-group"><button type="button" class="jmvl-cat" data-cat="ttests">T-Tests</button>\n            <div class="jmvl-menu"><button type="button" data-an="ttest_ind">Independent Samples T-Test</button><button type="button" data-an="ttest_paired">Paired Samples T-Test</button><button type="button" data-an="ttest_one">One Sample T-Test</button></div></div>\n          <div class="jmvl-group"><button type="button" class="jmvl-cat" data-cat="anova">ANOVA</button>\n            <div class="jmvl-menu"><button type="button" data-an="anova_oneway">One-Way ANOVA</button><button type="button" data-an="kruskal">Kruskal-Wallis</button></div></div>\n          <div class="jmvl-group"><button type="button" class="jmvl-cat" data-cat="regression">Regression</button>\n            <div class="jmvl-menu"><button type="button" data-an="correlation">Correlation Matrix</button><button type="button" data-an="lin_reg">Linear Regression</button><button type="button" data-an="log_reg">Logistic Regression</button></div></div>\n          <div class="jmvl-group"><button type="button" class="jmvl-cat" data-cat="frequencies">Frequencies</button>\n            <div class="jmvl-menu"><button type="button" data-an="contingency">Contingency Tables (χ²)</button><button type="button" data-an="gof">χ² Goodness of Fit</button></div></div>';
      rib.innerHTML =
        '<div class="jmvl-tabbar">'
        + '<button type="button" class="jmvl-hamburger" title="Meny" aria-label="Meny">☰</button>'
        + '<button type="button" class="jmvl-tab" data-jtab="variables">Variabler</button>'
        + '<button type="button" class="jmvl-tab" data-jtab="data">Data</button>'
        + '<button type="button" class="jmvl-tab active" data-jtab="analyses">Analyser</button>'
        + '<button type="button" class="jmvl-tab" data-jtab="figures">Figurer</button>'
        + '<button type="button" class="jmvl-tab" data-jtab="edit">Rediger</button>'
        + '<div class="jmvl-app-menu" hidden><button type="button" data-jaction="examples">' + T('Åpne eksempeldatasett…') + '</button><button type="button" data-jaction="clear">' + T('Tøm resultater') + '</button><button type="button" data-jaction="about">' + T('Om jamovi-modus') + '</button></div>'
        + '<div class="jmvl-dataset-picker"><label for="jamoviLightDatasetSelect">Aktivt datasett:</label><select id="jamoviLightDatasetSelect"></select></div>'
        + '</div>'
        + '<div class="jmvl-ribbon-area">'
        + '<div class="jmvl-panel" data-jpanel="analyses">' + catGroups + '</div>'
        + '<div class="jmvl-panel" data-jpanel="variables" hidden><button type="button" class="jmvl-ribbon-btn" data-jaction="show-variables">' + T('Vis variabler') + '</button><span class="jmvl-ribbon-hint">' + T('Måltype for hver variabel i det aktive datasettet.') + '</span></div>'
        + '<div class="jmvl-panel" data-jpanel="data" hidden>'
        +   '<button type="button" class="jmvl-ribbon-btn" data-jaction="show-data">Vis data</button>'
        +   '<button type="button" class="jmvl-ribbon-btn" data-jaction="compute-var">Beregn variabel</button>'
        +   '<button type="button" class="jmvl-ribbon-btn" data-jaction="recode-var">Omkod variabel</button>'
        +   '<button type="button" class="jmvl-ribbon-btn" data-jaction="filter">Filter</button>'
        +   '<button type="button" class="jmvl-ribbon-btn" data-jaction="add-row">Legg til rad</button>'
        +   '<button type="button" class="jmvl-ribbon-btn" data-jaction="delete-row">Slett valgt rad</button>'
        + '</div>'
        + '<div class="jmvl-panel" data-jpanel="figures" hidden>'
        +   '<button type="button" class="jmvl-ribbon-btn" data-an="fig_histogram">' + (JAMOVI_ICONS.descriptives||'') + '<span>Histogram</span></button>'
        +   '<button type="button" class="jmvl-ribbon-btn" data-an="fig_boxplot"><span>Box Plot</span></button>'
        +   '<button type="button" class="jmvl-ribbon-btn" data-an="fig_barplot">' + (JAMOVI_ICONS.frequencies||'') + '<span>Bar Plot</span></button>'
        +   '<button type="button" class="jmvl-ribbon-btn" data-an="fig_scatter">' + (JAMOVI_ICONS.correlation||'') + '<span>Scatter Plot</span></button>'
        + '</div>'
        + '<div class="jmvl-panel" data-jpanel="edit" hidden><span class="jmvl-ribbon-hint">Data redigeres via skript (Microdata/Python/R/Stata), ikke direkte i jamovi-modus.</span></div>'
        + '</div>';
      bar.appendChild(rib);
    }
    // Wire ribbon (initJamoviRibbon logic, inline not as IIFE)
    (function initJamoviRibbon() {
      var rib = document.getElementById('jamoviLightRibbon');
      if (!rib) return;
      var apanel = rib.querySelector('.jmvl-panel[data-jpanel="analyses"]');
      // Analyses ribbon: category icons + dropdown toggles + analysis clicks
      apanel.querySelectorAll('.jmvl-cat').forEach(function(btn){ var c = btn.getAttribute('data-cat'); btn.innerHTML = (JAMOVI_CAT_ICONS[c]||'') + '<span>' + btn.textContent + '</span>'; });
      apanel.querySelectorAll('.jmvl-cat').forEach(function(btn){
        btn.addEventListener('click', function(e){
          e.stopPropagation();
          var g = btn.parentElement, wasOpen = g.classList.contains('open');
          apanel.querySelectorAll('.jmvl-group').forEach(function(x){ x.classList.remove('open'); });
          if (!wasOpen) g.classList.add('open');
        });
      });
      apanel.querySelectorAll('.jmvl-menu button[data-an]').forEach(function(b){
        var an = b.getAttribute('data-an');
        b.innerHTML = (JAMOVI_ICONS[an] || '') + '<span>' + b.textContent + '</span>';
        b.addEventListener('click', function(){ apanel.querySelectorAll('.jmvl-group').forEach(function(x){x.classList.remove('open');}); openJamoviAnalysis(an); });
      });
      // Tab bar
      function switchTab(tab){
        rib.querySelectorAll('.jmvl-tab').forEach(function(t){ t.classList.toggle('active', t.getAttribute('data-jtab')===tab); });
        rib.querySelectorAll('.jmvl-panel').forEach(function(p){ p.hidden = (p.getAttribute('data-jpanel')!==tab); });
        M.updateModeGuiBar(); // bar height changes with the panel
        if (tab==='variables') renderVariablesView();
        else if (tab==='data') renderDataView();
      }
      rib.querySelectorAll('.jmvl-tab').forEach(function(t){
        t.addEventListener('click', function(e){ e.stopPropagation(); switchTab(t.getAttribute('data-jtab')); });
      });
      // Hamburger application menu
      var ham = rib.querySelector('.jmvl-hamburger');
      var appMenu = rib.querySelector('.jmvl-app-menu');
      ...
      appMenu.querySelectorAll('button[data-jaction]').forEach(function(b){
        b.addEventListener('click', function(e){
          ...
          if (act === 'clear') { var c = M.outputArea.querySelector('#jamoviLightResults'); if (c) c.innerHTML = ''; }
      ...
      rib.querySelectorAll('.jmvl-ribbon-btn[data-jaction]').forEach(function(b){
      ...
      rib.querySelectorAll('.jmvl-ribbon-btn[data-an]').forEach(function(b){
        b.addEventListener('click', function(){ openJamoviAnalysis(b.getAttribute('data-an')); });
      });
      // Active-dataset picker
      var dsel = rib.querySelector('#jamoviLightDatasetSelect');
      if (dsel) {
      ...
      document.addEventListener('click', function(){ apanel.querySelectorAll('.jmvl-group').forEach(function(x){x.classList.remove('open');}); if (appMenu) appMenu.hidden = true; });
    })();

    M.registerMode({ id:'jamovilight', label:'Jamovi light', hlConfig:M.R_HL_CFG, handleTab:M.handleRTab, topGui:'jamovilight', onActivate:function(){ if(!M.isWebRReady()) M.loadWebR(); M.updateModeGuiBar(); jamoviRefreshDatasetPicker(); }, translate:{showsButton:false}, runSelf:async function(script,ctx){ await M.runHybridR(script, ctx.py, {showCommands:true}); } });
    M.updateModeGuiBar();
})();
